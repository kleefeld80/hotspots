function HelmDL2alphaBrickTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    for i=1:2*faces
      ...
      L=L+3;
      s=alpha;
      l1(s)*x1+l2(s)*x2+l3(s)*x3
      l1(s)*y1+l2(s)*y2+l3(s)*y3
      s=0.5;
      l1(s)*x1+l2(s)*x2+l3(s)*x3
      l1(s)*y1+l2(s)*y2+l3(s)*y3
      s=1-alpha;
    ...
    if yes
       ...
       save('brick.mat','z')
       plotthething2('brick.eps',xx,yy,real(z),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
