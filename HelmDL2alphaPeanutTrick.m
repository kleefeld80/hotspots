function HelmDL2alphaD3EllipseTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    ii=0:1:n;
    ti=2*pi*ii/n;
    vi=sqrt(cos(ti).*cos(ti)+sin(ti).*sin(ti)/4).*[cos(ti);sin(ti)];
    
    nnn=100;
    ii=0:1:nnn;
    ti=2*pi*ii/nnn;
    outer=sqrt(cos(ti).*cos(ti)+sin(ti).*sin(ti)/4).*[cos(ti);sin(ti)];
    ...
    if yes
        ...
        x=linspace(-1.3,1.3,reso);
        y=linspace(-1.3,1.3,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
        ...
        save('peanut.mat','z')
       plotthething('peanut.eps',xx,yy,real(z),outer,12,[-1,-0.5,0,0.5,1],[-1,-0.5,0,0.5,1])
