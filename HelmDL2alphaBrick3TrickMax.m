function HelmDL2alphaBrick3TrickMax(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    p19=[3;6]*(1-s)+[0;6]*s;
    p20=[0;6]*(1-s)+[0;1]*s;
    vi1=([p1(:,1:n-1) p2(:,1:n-1) p3(:,1:n-1) p4(:,1:n-1) p5(:,1:n-1)...
        p6(:,1:n-1) p7(:,1:n-1) p8(:,1:n-1) p9(:,1:n-1) p10(:,1:n-1)...
        p11(:,1:n-1) p12(:,1:n-1) p13(:,1:n-1) p14(:,1:n-1) p15(:,1:n-1)...
        p16(:,1:n-1) p17(:,1:n-1) p18(:,1:n-1) p19(:,1:n-1) p20]-[5;3.5])/2;
    ...
    p19=[3;5]*(1-s)+[1;5]*s;
    p20=[1;5]*(1-s)+[1;2]*s;
    vi2=fliplr(([p1(:,1:n-1) p2(:,1:n-1) p3(:,1:n-1) p4(:,1:n-1) p5(:,1:n-1)...
        p6(:,1:n-1) p7(:,1:n-1) p8(:,1:n-1) p9(:,1:n-1) p10(:,1:n-1)...
        p11(:,1:n-1) p12(:,1:n-1) p13(:,1:n-1) p14(:,1:n-1) p15(:,1:n-1)...
        p16(:,1:n-1) p17(:,1:n-1) p18(:,1:n-1) p19(:,1:n-1) p20]-[5;3.5])/2);
    ...
    [x,mai]=fminsearch(@computevalue,[0;1],options)
    [x,mii]=fminsearch(@computevalue2,[0;-1],options)
    ...
    if yes
        ...
        x=linspace(-3,3,reso);
        y=linspace(-3,3,reso);
        %x=linspace(-2,3.5,reso);
        %y=linspace(-2,3.5,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
       ...
       save('brick2.mat','z')
       plotthething2('brick2.eps',xx,yy,real(z),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
