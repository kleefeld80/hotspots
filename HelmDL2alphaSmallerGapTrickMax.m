function HelmDL2alphaSmallerGapTrickMax(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    %[xx,yy]=meshgrid(x,y);
    %X=load('complexSmallerGap');
    %plotthething2('complexSmallerGap.eps',xx,yy,real(X.z),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
      
    %plotthething2('complexSmallerGap2.eps',xx(50:100,40:60),yy(50:100,40:60),real(X.z(50:100,40:60)),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
    %pause
    ...
    %iii=input('Number:');
    iii=1;
    eigenwert=eigenwerte(iii);
    eigenvector=V0(:,iii);
    options = optimset('TolFun',1e-10,'TolX',1e-10);
    [x,mai]=fminsearch(@computevalue,[0;0.6],options)
    [x,mii]=fminsearch(@computevalue2,[0;-0.6],options)
    mar=max(real(eigenvector(:)))
    mir=min(real(eigenvector(:)))
    mai/mar
    mii/mir
    pause
    ...
    if yes
       ...
       save('complexSmallerGap.mat','z')
       plotthething2('complexSmallerGap.eps',xx,yy,real(z),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
...
function val=computevalue(P)
    ...
    for jj=1:n/2*2    % number of faces
      ...
      y3=vi(2,indi3(jj));
      k=k+2;
      E1=intDL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwert,1);
      E2=intDL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwert,2);
      E3=intDL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwert,3);

      summ=summ+E1*eigenvector(L)+E2*eigenvector(L+1)+E3*eigenvector(L+2);
      L=L+3;
    end
    val=-real(summ);
end

function val=computevalue2(P)
    global vi
    global n
    global eigenwert
    global eigenvector
