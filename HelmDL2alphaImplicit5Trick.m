function HelmDL2alphaImplicit5Trick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    if yes
        ...
        save('implicit5.mat','z')
       plotthething('implicit5.eps',xx,yy,real(z),outer,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
...
function val=pot4(r)
    ...
    x=r*cos(p);
    y=r*sin(p);
    val=1/((x+3/2)^2+(y-0.1)^2)^alp+...
        1/((x-3/2)^2+(y)^2)^alp+...
        1/((x-        0.1)^2+(y+sqrt(3)/2+0.1)^2)^alp+...
        1/((x-        0)^2+(y-sqrt(3)/2)^2)^alp-c;
end
