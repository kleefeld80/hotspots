function err=HelmDL2alphAnn6(ns)
    global alpha
       global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    a=0.35;
    b=0.8;
    vi2=fliplr([(0.55+0.6*cos(ti)+0.2*cos(2*ti))*0.6;0.7*sin(ti)]);
    a=0.35;
    b=0.8;
    vi3=[-(0.55+0.6*cos(ti)+0.2*cos(2*ti))*0.6;0.7*sin(ti)];
    vi=[vi1,vi2,vi3];
    %vi=geometrie(4);
    n=size(vi,2)/3-1
    faces=n/2;
    ...
    indi1=[1:2:n  ,n+2:2:2*n,  2*n+3:2:3*n+1];
    indi2=[2:2:n  ,n+3:2:2*n+1,2*n+4:2:3*n+2];
    indi3=[3:2:n+1,n+4:2:2*n+2,2*n+5:2:3*n+3];
    ...
    if yes
        ...
        for i=1:reso
            for j=1:reso
                ...
                [In3,On3]=inpolygon(x,y,inner2(1,:),inner2(2,:));
                In=1;
                if In==1 %&& In2==0 && In3==0
                  P=[x;y];
                  summ=0;
                  k=1;
                  L=1;
                  for jj=1:n/2*3    % number of faces
...
function A=matr(k)
  ...
  A = createDmatrix();
  %n=size(A,1)/3;
  %A(1:n,1:n)=A(1:n,1:n)+0.5*diag(ones(n,1));
  %A(n+1:3*n,n+1:3*n)=A(n+1:3*n,n+1:3*n)-0.5*diag(ones(2*n,1));
  A=A+0.5*eye(size(A));
end
