function HelmDL2alpha2HolesTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    vhi=geometrie2holes(31);
    nnn=length(vhi)/3;
    % scale the points
    vhi=vhi*0.25;
    outer =vhi(:,  1:  nnn);
    inner =vhi(:,nnn+1:2*nnn);
    inner2=vhi(:,2*nnn+1:end);
    ...
    if yes
       ...
       %mai/mar
       %mii/mir
       plotthething2holes('complex2holes3f.eps',xx,yy,real(z),outer,inner,inner2,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
       pause
       x=linspace(-0.6,0.6,reso);
        ...
        save('complex2.mat','z')
       plotthething3('complex2.eps',xx,yy,real(z),outer,inner,13,[-0.5,0,0.5],[0.5,1])
       x=linspace(-0.6,0.6,reso);
