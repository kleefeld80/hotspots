function HelmDL2alph5HolesTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    vhi=geometrie5holes(31);
    nnn=length(vhi)/6;
    % scale the points
    vhi=vhi*0.25;
    outer =vhi(:,      1:  nnn);
    inner =vhi(:,  nnn+1:2*nnn);
    inner2=vhi(:,2*nnn+1:3*nnn);
    inner3=vhi(:,3*nnn+1:4*nnn);
    inner4=vhi(:,4*nnn+1:5*nnn);
    inner5=vhi(:,5*nnn+1:end);
    
    %nnn=length(vhi)/3-1;
    % scale the points
    %outer =vi(:,  1:  nnn+1);
    %inner =vi(:,nnn+2:2*nnn+2);
    %inner2=vi(:,2*nnn+3:end);
    %plotthething5('geocomplex.eps',outer,inner,10,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
    %pause   
    
    indi1=[1:2:n  ,n+2:2:2*n,  2*n+3:2:3*n+1,3*n+4:2:4*n+2,4*n+5:2:5*n+3,5*n+6:2:6*n+4];
    indi2=[2:2:n  ,n+3:2:2*n+1,2*n+4:2:3*n+2,3*n+5:2:4*n+3,4*n+6:2:5*n+4,5*n+7:2:6*n+5];
    indi3=[3:2:n+1,n+4:2:2*n+2,2*n+5:2:3*n+3,3*n+6:2:4*n+4,4*n+7:2:5*n+5,5*n+8:2:6*n+6];
    vhi=vi;
    k=1;
    L=1;
    for i=1:6*faces
    ...
    if yes
        ...
        x=linspace(-1.8,1.8,reso);
        y=linspace(-1.8,1.8,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
            for j=1:reso
                ...
                [In3,~]=inpolygon(x,y,inner2(1,:),inner2(2,:));
                [In4,~]=inpolygon(x,y,inner3(1,:),inner3(2,:));
                [In5,~]=inpolygon(x,y,inner4(1,:),inner4(2,:));
                [In6,~]=inpolygon(x,y,inner5(1,:),inner5(2,:));
                %In=1;
                if In==1 && In2==0 && In3==0 && In4==0 && In5==0 && In6==0
                  P=[x;y];
                  summ=0;
                  k=1;
                  L=1;
                  for jj=1:n/2*6    % number of faces
       ...
       %mai/mar
       %mii/mir
       plotthething3holes('complex5holes3.eps',xx,yy,real(z),outer,inner,inner2,inner3,inner4,inner5,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
       pause
       x=linspace(-0.6,0.6,reso);
        y=linspace(0.1,1.3,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
        ...
        save('complex3.mat','z')
       plotthething3('complex3.eps',xx,yy,real(z),outer,inner,13,[-0.5,0,0.5],[0.5,1])
       x=linspace(-0.6,0.6,reso);
        y=linspace(-1.3,-0.1,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
        ...
        save('complex3.mat','z')
       plotthething4('complex3.eps',xx,yy,real(z),outer,inner,14,[-0.5,0,0.5],[-1,-0.5])
    end
    pause
    end
    end
...
function A=createDmatrix()
  ...
  % create the matrix of size n x n
    A=zeros(3*faces*6,3*faces*6);
    M0=zeros(3*faces*6,3*faces*6);
    for i=1:3*faces*6
