...
% n= 12 ns=  3   2.424210214149178 + 0.004688587127143i 0.007071208764926
% n= 24 ns=  5   2.426413120797112 + 0.000260414636336i 0.003101357199697     1.189057154624568
% n= 48 ns=  9   2.428306014523190 + 0.000010359365044i 0.001197555698294     1.372806941089274
% n= 96 ns= 17   2.429028521133753 + 0.000000406542287i 4.750044544166939e-04 1.334079809560328
% n=192 ns= 33   2.429314921736158 + 0.000000015928511i 1.886036787095900e-04 1.332583226554636
% n=384 ns= 65   2.429428653824225 + 0.000000000624655i 7.487158997226633e-05 1.332867519072531
% n=768 ns=129   2.429473800556782 + 0.000000000024524i 2.972485741282628e-05 1.332748502306118

% Exact          2.429503525414195 (3 digits)
function HelmDL2alphaLshapeTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    p5=[1;2]*(1-s)+[0;2]*s;
    p6=[0;2]*(1-s)+[0;0]*s;
    vi=[p1(:,1:n-1) p2(:,1:n-1) p3(:,1:n-1) p4(:,1:n-1) p5(:,1:n-1) p6]/2;
    n=length(vi)-1
    faces=n/2;
    outer=vi;
    ...
    abs(eigenwerte(1)-sqrt(1.4756218450)*2)
    ...
    if yes
        ...
        x=linspace(-0.1,1.1,reso);
        y=linspace(-0.1,1.1,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
        ...
        save('lshape.mat','z')
       plotthething('lshape.eps',xx,yy,real(z),outer,12,[-1,-0.5,0,0.5,1],[-1,-0.5,0,0.5,1])
