function err=HelmDL2alphAnn7(ns)
    global alpha
       global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    ii=0:1:n;
    ti=2*pi*ii/n;
    vi1=[a*cos(ti);b*sin(ti)]; % Circle and Ellipse
    vi1=[0.75*cos(ti)+0.3*cos(2*ti);sin(ti)];
    a=0.3;
    b=0.3;
    vi2=fliplr([-0.1+a*cos(ti);-0.6+b*sin(ti)]);
    a=0.3;
    b=0.3;
    vi3=fliplr([-0.1+a*cos(ti); 0.6+b*sin(ti)]);
    a=0.4;
    b=0.5;
    vi4=fliplr([0.5+a*cos(ti);b*sin(ti)]);
    vi=[vi1,vi2,vi3,vi4];
    %vi=geometrie(4);
    n=size(vi,2)/4-1
    faces=n/2;
    ...
    outer =vi(:,        1:   n+1);
    inner =vi(:,   n+1 +1:2*(n+1));
    inner2=vi(:,2*(n+1)+1:3*(n+1));
    inner3=vi(:,3*(n+1)+1:end);
    
    %plot(inner(1,:),inner(2,:))
    %[In,On]=inpolygon(1,1,outer(1,:),outer(2,:))
    %pause
...
%     end
    ...
    if yes
        ...
        x=linspace(-1.1,1.1,reso);
        y=linspace(-1.1,1.1,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
            for j=1:reso
                ...
                %[In2,On2]=inpolygon(x,y,inner(1,:),inner(2,:));
                In=1;
                if In==1 %&& In2==0 
                  P=[x;y];
                  summ=0;
                  ...
                  for jj=1:n/2*4    % number of faces
                      % extract the first, middle, and endpoint
                          ...
                          y3=vi(2,indi3(jj));
                      k=k+2;
                      E1=intDL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwerte1,1,0);
                      E2=intDL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwerte1,2,0);
                      E3=intDL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwerte1,3,0);
        ...
        line(inner (1,:),inner (2,:),'Color','k','Linewidth',2)
        line(inner2(1,:),inner2(2,:),'Color','k','Linewidth',2)
        line(inner3(1,:),inner3(2,:),'Color','k','Linewidth',2)
        colorbar()
