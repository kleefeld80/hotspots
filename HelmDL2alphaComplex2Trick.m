function HelmDL2alphaComplex2Trick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    ii=0:1:n;
    ti=2*pi*ii/n;
    ppp=sin(ww*ti).*sin(ww*ti);
    ppp(ti<(ww/2-1)*pi/ww | ti>(3*ww/2+1)*pi/ww |(ti>(ww/2+1)*pi/ww & ti<(3*ww/2-1)*pi/ww))=0;
    vi1=[(1+0.5*ppp).*a.*cos(ti);(1+0.5*ppp).*a.*sin(ti)]; % Circle and Ellipse
    vi2=[(1-0.5*ppp).*b.*cos(ti);(1-0.5*ppp).*b.*sin(ti)];
    vi=[vi1,vi2];
    n=size(vi,2)/2-1;
    faces=n/2;
    ...
    ppp=sin(ww*ti).*sin(ww*ti);
    ppp(ti<(ww/2-1)*pi/ww | ti>(3*ww/2+1)*pi/ww |(ti>(ww/2+1)*pi/ww & ti<(3*ww/2-1)*pi/ww))=0;
    outer=[(1+0.5*ppp).*a.*cos(ti);(1+0.5*ppp).*a.*sin(ti)]; % Circle and Ellipse
    inner=[(1-0.5*ppp).*b.*cos(ti);(1-0.5*ppp).*b.*sin(ti)];
    ...
    if yes
       ...
       save('complexAusfluss3.mat','z')
       plotthething2('complexAusfluss3.eps',xx,yy,real(z),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
    end
    pause
    end
    end
