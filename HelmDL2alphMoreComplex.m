function err=HelmDL2alphMoreComplex(ns)
    global alpha
       global l
  global m
  global N
...
%     end
    ...
    if yes
        ...
        x=linspace(-0.5,0.5,reso);
        y=linspace(-0.8,0.8,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
...
function A=matr(k)
  ...
  %A(n+1:2*n,n+1:2*n)=A(n+1:2*n,n+1:2*n)-0.5*diag(ones(n,1));
  A=A+0.5*eye(size(A));
end
...
function E=intDL(x1s,x2s,x3s,y1s,y2s,y3s,xhs,yhs,ks,basisf,check)
  ...
  if check==0
    E=quadgk(@c11_1,0,1,'AbsTol',1e-6);
  elseif check==1
    E=quadgk(@c11_1,0,alpha,'AbsTol',1e-6)+quadgk(@c11_1,alpha,1,'AbsTol',1e-6);
  elseif check==2
    E=quadgk(@c11_1,0,0.5,'AbsTol',1e-6)+quadgk(@c11_1,0.5,1,'AbsTol',1e-6);
  elseif check==3
    E=quadgk(@c11_1,0,1-alpha,'AbsTol',1e-6)+quadgk(@c11_1,1-alpha,1,'AbsTol',1e-6);
  else
