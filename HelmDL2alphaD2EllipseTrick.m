function HelmDL2alphaD2EllipseTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    ii=0:1:n;
    ti=2*pi*ii/n;
    vi=[0.75*cos(ti)+epsi*cos(2*ti);sin(ti)];
    
    nnn=100;
    ii=0:1:nnn;
    ti=2*pi*ii/nnn;
    outer=[0.75*cos(ti)+epsi*cos(2*ti);sin(ti)];
    ...
    if yes
        ...
        save('deformed2.mat','z')
       plotthething('deformed2.eps',xx,yy,real(z),outer,12,[-1,-0.5,0,0.5,1],[-1,-0.5,0,0.5,1])
