% n has to be even
% seem to work well
function err=HelmS2alphaAnnulus(ns)
    if mod(ns,2)
        fprintf('n has to be even\n');
        return
    end
    global alpha
    global l
  ...
  wavenumber=1;
    ...
    for jj=1:n/2*2    % number of faces
        % extract the first, middle, and endpoint
            ...
            y3=vi(2,indi3(jj));
        k=k+2;
        E1=intSL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwerte,1,0);
        E2=intSL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwerte,2,0);
        E3=intSL(x1,x2,x3,y1,y2,y3,P(1),P(2),eigenwerte,3,0);

        summ=summ+E1*V0(L)+E2*V0(L+1)+E3*V0(L+2);
        L=L+3;
    end
                z(i,j)=summ;
            end
...
x1=vi(1,indi1(j));
            ...
            y3=vi(2,indi3(j));
        k=k+2;
        E1=intSL(x1,x2,x3,y1,y2,y3,P(1),P(2),wavenumber,1,0);
        E2=intSL(x1,x2,x3,y1,y2,y3,P(1),P(2),wavenumber,2,0);
        E3=intSL(x1,x2,x3,y1,y2,y3,P(1),P(2),wavenumber,3,0);
  ...
  % create the matrix of size n x n
    ...
    for i=1:3*faces*2
        ...
        for j=1:n/2*2    % number of faces
            ...
            k=k+2;
            if i==L
              E1=intSL(x1,x2,x3,y1,y2,y3,xh,yh,wavenumber,1,1);
            else
              E1=intSL(x1,x2,x3,y1,y2,y3,xh,yh,wavenumber,1,0);
            end
            if i==L+1
              E2=intSL(x1,x2,x3,y1,y2,y3,xh,yh,wavenumber,2,2);
            else
              E2=intSL(x1,x2,x3,y1,y2,y3,xh,yh,wavenumber,2,0);
            end
            if i==L+2
              E3=intSL(x1,x2,x3,y1,y2,y3,xh,yh,wavenumber,3,3);
            else
              E3=intSL(x1,x2,x3,y1,y2,y3,xh,yh,wavenumber,3,0);
            end
