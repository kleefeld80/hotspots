function err=HelmDL2alphAnn5(ns)
    global alpha
       global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    ii=0:1:n;
    ti=2*pi*ii/n;
    vi1=[0.75*cos(ti)+0.3*cos(2*ti);sin(ti)]; % Circle and Ellipse
    a=0.15;
    b=0.35;
    vi2=0.5*[sin(ti)+0.5;0.75*cos(ti)+0.3*cos(2*ti)];
    vi=[vi1,vi2];
    %vi=geometrie(4);
    n=size(vi,2)/2-1
    faces=n/2;
...
%     end
    ...
    if yes
        ...
        x=linspace(-1,1.2,reso);
        y=linspace(-1,1.2,reso);
        [xx,yy]=meshgrid(x,y);
        z=xx;
        %z=0;
        for i=1:reso
