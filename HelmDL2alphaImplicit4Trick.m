function HelmDL2alphaImplicit4Trick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    if yes
        ...
        save('implicit4.mat','z')
       plotthething('implicit4.eps',xx,yy,real(z),outer,12,[-1.5,-1,-0.5,0,0.5,1,1.5],[-1.5,-1,-0.5,0,0.5,1,1.5])
...
function val=pot4(r)
    ...
    x=r*cos(p);
    y=r*sin(p);
    val=1/((x+5/4)^2+(y-1/10)^2)^alp+...
        1/((x-5/4)^2+(y)^2)^alp+...
        1/((x-        1/10)^2+(y+1)^2)^alp+...
        1/((x-        0)^2+(y-1)^2)^alp-c;
end
