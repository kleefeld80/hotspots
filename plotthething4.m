...
function plotthething4(filename,xx,yy,Z,outer,inner,fignr,xticksvec,yticksvec)
    terrain = [0 166 0
        22 174 0
        45 182 0
        71 190 0
    ...
    figure(fignr)
       ...
       set(gca,'fontsize',ticks,'fontweight','bold','LineWidth',2);
        ...
        cmin=min(Z(:));
        cmax=max(Z(:));
        [row1, col1] = find(ismember(Z, cmin));
        [row2, col2] = find(ismember(Z, cmax));
        xmin=xx(row1,col1);
        ymin=yy(row1,col1);
        xmax=xx(row2,col2);
        ymax=yy(row2,col2);
