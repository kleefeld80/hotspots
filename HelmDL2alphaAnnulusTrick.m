function HelmDL2alphaAnnulusTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    ii=0:1:n;
    ti=2*pi*ii/n;
    vi1=[a*cos(ti);b*sin(ti)];
    a=0.5;
    b=0.5;
    vi2=fliplr([a*cos(ti);b*sin(ti)]);
    vi=[vi1,vi2];
    n=size(vi,2)/2-1;
    faces=n/2;
    ...
    ii=0:1:nnn;
    ti=2*pi*ii/nnn;
    outer=[a*cos(ti);b*sin(ti)];
    a=0.5;
    b=0.5;
    inner=fliplr([a*cos(ti);b*sin(ti)]);
    ...
    eigenwerte
    abs(eigenwerte(1)-0.8222526886238843)
    abs(eigenwerte(1)-1.504647782189479)
    abs(eigenwerte(3)-0.8222526886238843)
    abs(eigenwerte(3)-1.504647782189479)
    ...
    if yes
       ...
       save('annulus1.mat','z')
       plotthething2('annulus1.eps',xx,yy,real(z),outer,inner,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
