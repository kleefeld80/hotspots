function HelmDL2alpha3HolesTrick(ns)
    global alpha
  global l
  global m
  global N
  ...
  wavenumber=1;
    ...
    indi1=[1:2:n  ,n+2:2:2*n,  2*n+3:2:3*n+1,3*n+4:2:4*n+2];
    indi2=[2:2:n  ,n+3:2:2*n+1,2*n+4:2:3*n+2,3*n+5:2:4*n+3];
    indi3=[3:2:n+1,n+4:2:2*n+2,2*n+5:2:3*n+3,3*n+6:2:4*n+4];
    vhi=vi;
    k=1;
    L=1;
    for i=1:4*faces
    ...
    if yes
        ...
        for i=1:reso
            for j=1:reso
                ...
                %In=1;
                if In==1 && In2==0 && In3==0 && In4==0 
                  P=[x;y];
                  summ=0;
                  k=1;
                  L=1;
                  for jj=1:n/2*4    % number of faces
       ...
       %mai/mar
       %mii/mir
       plotthething3holes('complex3holes1f.eps',xx,yy,real(z),outer,inner,inner2,inner3,12,[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2],[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2])
       pause
...
function A=createDmatrix()
  ...
  % create the matrix of size n x n
    A=zeros(3*faces*4,3*faces*4);
    M0=zeros(3*faces*4,3*faces*4);
    for i=1:3*faces*4
